...
<body>
    <div class="avaliacao-container">
        ...
        <p class="avaliacao-descricao">
            Prezado cidadão, para melhor atender a sua demanda e aprimorar os serviços prestados por nossa gestão,
            gostaríamos que respondesse algumas perguntas para medir sua satisfação e, com isso, propor melhorias aos
            órgãos internos sobre os serviços avaliados.
        </p>
        <form method="POST" action="{{ route('avaliacoesperguntaStore') }}" class="avaliacao-form">
            @csrf
            <div class="form-group">
                <label for="cpf">Informe seu CPF</label>
                <input type="text" class="form-control" placeholder="Digite seu CPF" name="cpf" id="cpf" required>
                <small class="form-text">O CPF é necessário para limitar a votação mensal por pessoa. Os dados
                    informados nesta pesquisa estarão seguros de acordo com a LGPD.</small>
            </div>
            @for($i = 1; $i <= 14; $i++)
                    ...
                    14 => 'Como você avalia o atendimento e serviços da Assistência Social do nosso município?',
                            ...
                            <div class="nota-opcoes">
                                <label><input type="radio" name="resposta{{ $i }}" value="2" required> <span class="emoji">😞</span>
                                    Muito insatisfeito</label>
                                <label><input type="radio" name="resposta{{ $i }}" value="4"> <span class="emoji">☹️</span>
                                    Insatisfeito</label>
                                <label><input type="radio" name="resposta{{ $i }}" value="6"> <span class="emoji">😐</span>
                                    Neutro</label>
                                <label><input type="radio" name="resposta{{ $i }}" value="8"> <span class="emoji">😊</span>
                                    Satisfeito</label>
                                <label><input type="radio" name="resposta{{ $i }}" value="10"> <span class="emoji">😁</span> Muito
                                    satisfeito</label>
                            </div>
                        </div>
            @endfor
            ...
            <p class="avaliacao-privacidade">
                *Prezado(a), esta avaliação não será analisada como manifestação de Ouvidoria. Servirá apenas para
                revisarmos e refletirmos sobre as informações disponíveis nesta página.<br>
                Esclarecemos que os dados fornecidos acima serão tratados com respeito à sua privacidade.<br>
                Seguindo a <strong><a href="https://www.planalto.gov.br/ccivil_03/_ato2015-2018/2018/lei/l13709.htm"
                        target="_blank">LGPD - Lei Geral de Proteção de Dados 13.709</a></strong>, de 14 de agosto de
                2018.
            </p>
