...
<div id="modalAnexos{{ $manifestacao->id }}" class="modal" style="display: none;">
    <div class="modal-content">
        ...
        <div class="modal-header">
            ...
            <button class="close-btn" onclick="fecharModal({{ $manifestacao->id }})">&times;</button>
        </div>
        ...
        <div class="modal-body">
                        ...
                        <tr>
                            <td class="border p-2">
                                @if(in_array(strtolower($ext), ['jpg','jpeg','png','gif','webp']))
                                    <a href="#" onclick="mostrarImagem('{{ $url }}', '{{ $manifestacao->id }}'); return false;" style="color:#1976d2;">{{ $nome }}</a>
                                @else
                                    <a href="{{ $url }}" target="_blank" style="color:#1976d2;">{{ $nome }}</a>
                                @endif
                            </td>
                            <td class="border p-2 text-center">{{ $ext }}</td>
                            <td class="border p-2 text-center">
                                @if(in_array(strtolower($ext), ['jpg','jpeg','png','gif','webp']))
                                    <span id="imgPreview{{ $manifestacao->id }}{{ $loop->index }}"></span>
                                @else
                                    <a href="{{ $url }}" target="_blank" class="px-3 py-1 bg-blue-500 text-white rounded">Abrir</a>
                                @endif
                            </td>
                            <td class="border p-2 text-center">
                                <a href="{{ $url }}" download class="px-3 py-1 bg-green-600 text-white rounded">Baixar</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            ...
            <div id="imagemModal{{ $manifestacao->id }}" style="display:none; text-align:center; margin-top:20px;">
                <img id="imagemExibida{{ $manifestacao->id }}" src="" alt="Visualização" style="max-width:90%; max-height:60vh; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,0.2);">
            </div>
        </div>
    </div>
</div>
